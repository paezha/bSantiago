...
Santiago_H <- SantiagoSurvey_trans %>%
  ...
  mutate(r1A_STRESS = factor(r1A_STRESS, 
                             levels = c("1", "2", "3", "4", "5"), 
                             labels = c("VERY HIGH", "ABOVE AVERAGE", "AVERAGE", "BELOW AVERAGE", "VERY LOW"),
                             ordered = TRUE)) %>%
  mutate(r1B_EFFORT = factor(r1B_EFFORT, 
                             levels = c("1", "2", "3", "4", "5"), 
                             labels = c("VERY NEGATIVE", "SLIGHTLY NEGATIVE", "NEUTRAL", "MOSTLY POSITIVE", "COMPLETELY POSITVE"),
                             ordered = TRUE)) %>%
  ...
  mutate(r1D_CONTAM = factor(r1D_CONTAM, 
                             levels = c("1", "2", "3", "4", "5"), 
                             labels = c("VERY DISSATISFIED", "DISSATISFIED", "UNSURE", "SATISFIED", "VEERY SATISFIED"),
                             ordered = TRUE)) %>%
...
Santiago_FE <- SantiagoSurvey_trans %>%
  ...
  mutate(r2D_AFFECT = factor(r2D_AFFECT, 
                             levels = c("A", "B", "C", "D", "E", "F", "G", "H"), 
                             labels = c("LACK OF COMFORT", "LACK OF SAFETY", "POOR QUALITY OF STREETS", "CROWDNESS OF PASSENGERS", "DISABILITY OR REDUCED MOBILITY", "TRAVEL TIME", "ALL OF THEM", "OTHER"))) %>%
  mutate(r2E_FACILIT = factor(r2E_FACILIT, 
                             levels = c("A", "B", "C", "D", "E", "F", "G", "H"), 
                             labels = c("BETTER COMFORT", "BETTER SAFETY", "BETTER QULALITY ON STREETS", "LESS CROWDNESS OF PASSENGERS", "IMPROVEMENT ON ACCESIBILITY", "REDUCTION OF TIME TRAVEL", "ALL OF THEM", "OTHER"))) 
```
...
Santiago_RPD <- SantiagoSurvey_trans %>%
 ...
 mutate(r3DA_FAM = factor(r3DA_FAM, 
  ...
  mutate(r3F_ACCESS_DEPENDENCY = factor(r3F_ACCESS_DEPENDENCY, 
                             levels = c("1", "2", "3", "4", "5"), 
                             labels = c("VERY LOW", "BELOW AVERAGE", "AVERAGE", "ABOVE AVERAGE", "VERY HIGH"),
                             ordered = TRUE)) %>%
...
Santiago_IS <- SantiagoSurvey_trans %>%
  ...
  mutate(r6D_DIFFICULTY = factor(r6D_DIFFICULTY, 
                             ...
                             labels = c("Bad physical conection in sidewalks", "Shifts take too long", "Shifting is not comfortable", "Bad infrastructure for waiting times", "Shifting is not safe", "All the previous ones","Other"))) %>%
...
mutate(r8A_ACCESSJOB = factor(r8A_ACCESSJOB, 
                             levels = c("1", "2", "3", "4", "5"), 
                             labels = c("NO AFFECT", "MINOR AFFECT", "NEUTRAL", "MODERATE AFFECT", "MAJOR AFEECT"),
                             ordered = TRUE)) %>%
  ...
  mutate(r8D_EMPLSATISF = factor(r8D_EMPLSATISF, 
                             levels = c("1", "2", "3", "4", "5"), 
                             labels = c("NOT AT ALL SATISFIED", "SLIGHTLY SATISFIED", "MODERATELY SATISFIED", "VERY SATIAFIED", "EXTREMELY SATISFIED"),
                             ordered = TRUE)) %>%
...
```{r 1}
SantiagoSurvey_trans %>%                      # begin with linelist
  count(r0C_GENDER, r0J_INCOME) %>%     # group and tabulate counts by two columns
       mutate(r0C_GENDER = factor(r0C_GENDER, 
                             levels = c("A", "B"), 
                             labels = c("FEMALE", "MALE"))) %>%
  mutate(r0J_INCOME = factor(r0J_INCOME, 
  ...
  ggplot()+                       # pass new data frame to ggplot
    geom_col(                     # create bar plot
      mapping = aes(   
        x = r0J_INCOME,              # map outcome to x-axis
        fill = r0C_GENDER,           # map age_cat to the fill
        y = n))                   # map the counts column `n` to the height
```
```{r 2}

SantiagoSurvey_trans %>%                      # begin with linelist
  count(r0P_MODE1, r0J_INCOME) %>%     # group and tabulate counts by two columns
   mutate(r0P_MODE1  = factor(r0P_MODE1 , 
                             levels = c("A", "B", "C","D", "E", "F", "G", "H"), 
                             labels = c("CAR", "TAXI", "COLECTIVO", "MOTO", "METRO", "BUS","BICYCLE", "WALK"))) %>%
  mutate(r0J_INCOME = factor(r0J_INCOME, 
  ...
  ggplot()+                       # pass new data frame to ggplot
    geom_col(                     # create bar plot
      mapping = aes(   
        x = r0J_INCOME,              # map outcome to x-axis
        fill = r0P_MODE1,           # map age_cat to the fill
        y = n))                   # map the counts column `n` to the height

```

```{r 5}
ggplot(data = SantiagoSurvey_trans) + 
  (aes(x = r0C_GENDER,
       levels = c("A","B"),
labels = c("FEMALE", "MALE"),
       fill = r0P_MODE1)) +
  ...
  labs(y = "Proportion",
       x = "r1GA_STRESS", 
       fill = "r0P_MODE1") + 
  facet_wrap(~r1GA_STRESS)

SantiagoSurvey_trans$r0C_GENDER <- factor(SantiagoSurvey_trans$r0C_GENDER,
levels = c("A","B"),
labels = c("FEMALE", "MALE"))

SantiagoSurvey_trans$r0P_MODE1 <- factor(SantiagoSurvey_trans$r0P_MODE1,
levels = c( "A", "B", "C","D", "E", "F", "G", "H"),
labels = c("CAR", "TAXI", "COLECTIVO", "MOTO", "METRO", "BUS","BICYCLE", "WALK"))
```
